...
# 1. СТРАТЕГИ: Сонгогдсон цэсийг алдахгүй хадгалах
...
# 2. ДИЗАЙН (Таны хамгийн гоё гэсэн CSS загвар)
st.markdown("""
    <style>
    ...
    .custom-card { background: white; border-radius: 20px; padding: 30px; text-align: center; box-shadow: 0 10px 20px rgba(0,0,0,0.1); border: 1px solid #eee; height: 230px; margin-bottom: 10px; }
...
# 3. SIDEBAR (Цэс - Ямар ч үед харагдана)
with st.sidebar:
    ...
    # Сонгогдсон индексийг тогтвортой барих
    try:
    ...
    # Хэрэв цэс солигдсон бол session_state-ийг шинэчлэх
...
if st.session_state.selected_menu == "Нүүр хуудас":
    ...
    with c1:
        st.markdown('<div class="custom-card"><h2>📺</h2><h3>Цахим контент</h3><p>Видео хичээлүүд</p></div>', unsafe_allow_html=True)
        if st.button("Үзэх ➡️", key="nav_content", use_container_width=True):
    ...
    with c2:
        ...
        if st.button("Орох ➡️", key="nav_bank", use_container_width=True):
    ...
    with c3:
        ...
        if st.button("Эхлэх ➡️", key="nav_test", use_container_width=True):
            st.session_state.selected_menu = "Сорил"
            st.rerun()

# --- ДААЛГАВРЫН САН (Түвшингээр + Бөмбөлөг) ---
    ...
    if os.path.exists("data_bank.xlsx"):
        ...
        for lvl_name, tab in levels.items():
            with tab:
                ...
                if f_df.empty:
                    st.info(f"Энэ хэсэгт бодлого байхгүй байна.")
                else:
